...
SUMMARY_SIGNATURE = "_Суммаризировано @fast_summary_bot_"
...
async def handle_document(message: Message):
    ...
    try:
        ...
        # Предлагаем задать один вопрос по документу
        try:
            await message.answer(QUESTION_HINT)
        except Exception as e:
            logger.warning("Failed to send question hint: %s", e)
    ...
    if is_url(text):
        ...
        try:
            ...
            try:
                ...
                logger.warning("Failed to send question hint (link): %s", e)
...
async def handle_message(message: Message):
    ...
    # Если есть сохранённый документ для одноразового вопроса — используем его
    if message.text:
        pending = get_pending_document(message.from_user.id)
        if pending and pending.get("content"):
            try:
                answer = await answer_question_with_document(pending["content"], message.text)
                await message.answer(answer)
            except Exception as e:
                logger.exception("Failed to answer question for document %s", pending.get("id"))
                await message.answer("❌ Не удалось ответить на вопрос. Попробуйте позже.")
            finally:
                try:
                    mark_document_answered(pending["id"])
                except Exception as e:
